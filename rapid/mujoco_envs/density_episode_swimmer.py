...
class DensityEpisodeSwimmerEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        self.t = 0
        self.r = 0
        self.seed()
    ...
    def _init(self, model_path='swimmer.xml', frame_skip=4):
        ...
        # Read the XML
        density = str(self.np_random.randint(2000, 6000))
        with open(fullpath, 'r') as f:
            xml_text = f.read()
        xml_text = ET.fromstring(xml_text)
        xml_text.find('option').set('density', density)
